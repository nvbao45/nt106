...
import { getMyDishes } from '@/lib/api'
...
export default function MyDishesPage() {
  ...
  const fetchDishes = async (page = 1) => {
    ...
    setError('')
    try {
      const data = await getMyDishes(page, 12)
      setDishes(data.data)
      setPagination(data.pagination)
      setCurrentPage(page)
    } catch (err: any) {
      setError(err?.response?.data?.detail || 'Failed to fetch your dishes. Please login.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div>
      ...
      {!loading && !error && (
        <>
          {dishes.length === 0 ? (
            <p>You haven't contributed any dishes yet.</p>
          ) : (
            <>
              <div className="grid">
                {dishes.map((dish) => (
